  ...
  Q.Sprite.extend("Mario",{
   // Init se llama al crearlo
    ...
    step: function(dt) {
      ...
      if(!this.p.muerto){
	      if(this.p.y >= 600){
	        Q.stageScene("endGame",1, { label: "¡Te atrapó Meredith!" });
	        this.destroy();
	      }
...
var GLOBAL_STAGE;
  ...
  Q.scene('endGame',function(stage) {
    ...
    Q.audio.stop();
    if(stage.options.label !== "¡Te atrapó Meredith!"){
	...
	if(stage.options.label !== "¡Te atrapó Meredith!"){
    	...
    	var label = container.insert(new Q.UI.Text({
	      x:10, y: -30 - button.p.h, label: stage.options.label, size: Q.height/24, color: "rgb(232, 32, 117)"
	    }));
    }
  ...
  Q.UI.Text.extend("Coins",{
    init: function(p) {
      this._super(p,{
        label: "Corazones: "+Q.state.get("coins_collected"),
        color: "red",
        size: Q.height/30,
        x: 0,
        y: 0
